...
public class TradeService {
    ...
    public void sell(TradeController.TradeSellReq req){
        ...
        // 3. Trade에 쓸 참조 미리 꺼내기 (삭제 전에!)
        Users users = userStock.getUsers();
        Stock stock = userStock.getStock();

        // 4. 수익 / 금액 계산
        long profit = (req.price() - userStock.getAveragePrice()) * req.quantity();
        long amount = req.price() * req.quantity();

        // 5. 계좌 조회 + X-Lock → 예수금 증가
        ...
        // 7. 거래 내역 저장
        Trade trade = Trade.sell(users, stock, req.quantity(), req.price(), profit);
